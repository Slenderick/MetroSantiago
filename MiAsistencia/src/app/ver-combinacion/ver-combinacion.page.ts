...
@Component({
  selector: 'app-ver-combinacion',
  templateUrl: './ver-combinacion.page.html',
  styleUrls: ['./ver-combinacion.page.scss'],
})
export class VerCombinacionPage implements OnInit {
  listaEstaciones = []
  estaciones: Estacion[] = [];
  ...
  eliminarEstacion(estacion: Estacion) {
    if (estacion && estacion.nombre) {
      try {
        this.estacionesService.borrarCombinacion(estacion);
  ...
  obtenerEstacion() {
    this.estacionesService.obtenerNoCombinaciones("Si").subscribe((estaciones) => {
      if (estaciones) {
